...
默认所有能力都是开启的：文件写入审批、终端命令、系统进程控制、桌面控制、公网文本读取。不需要的能力可以在 Dashboard 中关闭。

可以在 Dashboard 中配置：

- 目录授权模式：授权所有目录 或 授权指定目录；
- 指定目录模式下每个目录单独设为只读（`ro`）或读写（`rw`）；
- 文件修改是否需要逐次本机审批；
- 终端命令；
- 系统进程控制；
- 桌面控制；
...
指定目录模式下，如果目录列表为空，则禁止全部文件访问。点击「从文件资源管理器选择文件夹」会在本机打开 Windows 文件夹选择窗口，逐个选择真实目录；已添加的目录可以单独切换只读/读写。

授权所有目录模式下，RDC-X 接受本机任意绝对路径。`.ssh`、`.env`、`.rdc` 等凭据路径和正在使用的 `tools\\tunnel-client.exe` 仍会被保护；此时「文件写入审批」几乎是唯一的写入闸门，除非完全信任本次会话，否则建议保持开启。

如果 RDC-X 工程自身位于明确授权的目录内，也允许通过 RDC-X 读取和修改自身源码。`.rdc` 私有运行状态、常见凭据文件/目录以及正在使用的 `tools\\tunnel-client.exe` 仍会被保护。

### 审批模式

**会话默认免审批**：文件修改、终端命令、系统进程、桌面控制和 Unity 修改都会直接执行，不再逐次弹窗确认。

在连接页点击「恢复逐次审批」可把当前会话切回逐次审批。该选择只保存在内存中：RDC-X 重启或本机策略变更后会回到默认的免审批状态。

即使处于免审批模式，通过 `set_config_value` 修改远程访问策略仍然必须经过本机明确审批。
