...
export class Approvals {
  ...
  prune() {
    for (const [id, item] of this.items) {
      if (item.status === 'pending' && item.expiresAt < Date.now()) {
        item.status = 'expired'; this.jobs.delete(id);
  ...
  async run(owner: string, action: string, preview: unknown, job: () => Promise<unknown>, requireApproval = true) {
    this.state.assertRunning(); this.prune();
    if (!requireApproval || this.mode(owner) === 'trusted') {
      if (this.mode(owner) === 'trusted') this.state.audit(action, 'session_approval_bypassed', undefined, owner);
    ...
    const id = random(16);
    this.items.set(id, { id, owner, action, preview, createdAt: Date.now(), expiresAt: Date.now() + 600000, status: 'pending' });
    this.jobs.set(id, job);
    this.state.audit(action, 'approval_required', { requestId: id, argumentHash: digest(JSON.stringify(preview)) }, owner);
    return { status: 'approval_required', requestId: id,
      instruction: 'Ask the computer owner to review this exact request in the LOCAL RDC-X dashboard. Then call get_request_result. Approval executes once; do not resubmit the original tool call.' };
  }
  private async perform(owner: string, action: string, job: () => Promise<unknown>) {
    this.state.assertRunning();
    try { const result = await job(); this.state.audit(action, 'completed', undefined, owner); return result; }
    catch (e: any) { this.state.audit(action, 'failed', { message: String(e.message).slice(0, 500) }, owner); throw e; }
  }
  async decide(id: string, approve: boolean) {
    ...
    this.jobs.delete(id);
    if (!approve) { item.status = 'rejected'; this.state.audit(item.action, 'rejected', { requestId: id }, item.owner); return; }
    item.status = 'running';
    try { item.result = await this.perform(item.owner, item.action, job); item.status = 'completed'; }
    catch (e: any) { item.status = 'failed'; item.error = e.message; }
  }
  result(id: string, owner: string) {
    ...
    const { preview, owner: _, ...rest } = item; return rest;
  }
